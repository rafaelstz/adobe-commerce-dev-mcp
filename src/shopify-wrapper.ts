...
interface ShopifyToolsOptions {
  onlyCode?: boolean;
}

export function shopifyTools(
  server: McpServer,
  options: ShopifyToolsOptions = {}
) {
    ...
    async ({ prompt }, extra) => {
      ...
      try {
        ...
        if (onlyCode) {
          ...
          console.error(
            `[shopify-gql] Using onlyCode mode, modified prompt: ${finalPrompt}`
          );
        }
