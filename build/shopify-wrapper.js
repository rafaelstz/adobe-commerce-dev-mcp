...
function extractContent(data) {
    console.error(`[extractContent] Attempting to extract content from: ${data.substring(0, 100) + (data.length > 100 ? "..." : "")}`);
    try {
...
export function shopifyTools(server, options = {}) {
    const { onlyCode = false } = options;
    server.tool("shopify-docs", "Search Shopify documentation", {
        ...
        try {
            // Modify prompt if onlyCode flag is set
            let finalPrompt = prompt;
            if (onlyCode) {
                finalPrompt +=
                    ". Instead of returning code and explanations, only return code.";
                console.error(`[shopify-gql] Using onlyCode mode, modified prompt: ${finalPrompt}`);
            }
            // Log the modified prompt in the logging message
            await server.server.sendLoggingMessage({
                level: "error",
                message: `[shopify-gql] Using${onlyCode ? " modified" : ""} prompt: ${finalPrompt}`,
            });
            ...
            console.error(`[shopify-gql] Making request to Shopify Dev with payload: ${JSON.stringify(requestPayload)}`);
            ...
            console.error(`[shopify-gql] Response status: ${response.status} ${response.statusText}`);
            // Convert headers to object in a compatible way
            const headersObj = {};
            response.headers.forEach((value, key) => {
                headersObj[key] = value;
            });
            console.error(`[shopify-gql] Response headers: ${JSON.stringify(headersObj)}`);
            if (!response.ok) {
            ...
            if (!contentType.includes("text/event-stream")) {
                ...
                console.error(`[shopify-gql] Response text (truncated): ${responseText.substring(0, 200) +
                    (responseText.length > 200 ? "..." : "")}`);
                const extractedContent = extractContent(responseText);
                console.error(`[shopify-gql] Extracted content: ${extractedContent
            ...
            const parser = createSSEParser((event) => {
                console.error(`[shopify-gql] Received event: ${event.event || "no-event"}`);
                if (event.data) {
                    // Extract content regardless of event type
                    const content = extractContent(event.data);
                    if (content) {
                        console.error(`[shopify-gql] Extracted content from event: ${content.substring(0, 100) +
                            (content.length > 100 ? "..." : "")}`);
                        responseText += content + "\n";
                        fullText += content;
                    }
                    else {
                        console.error(`[shopify-gql] No content extracted from event data`);
                    }
                }
                else {
            ...
            try {
                while (!done) {
                    ...
                    if (value) {
                        ...
                        console.error(`[shopify-gql] Received chunk #${chunkCount}, size: ${chunk.length} bytes`);
            ...
            console.error(`[shopify-gql] Final response text length: ${responseText.length}`);
            console.error(`[shopify-gql] Final response text (truncated): ${responseText.substring(0, 200) +
                (responseText.length > 200 ? "..." : "")}`);
            ...
            console.error(`[shopify-gql] Returning result structure: ${JSON.stringify(result, null, 2)}`);
            return result;
        }
        catch (error) {
            console.error(`[shopify-gql] Error communicating with Shopify Dev: ${error}`);
            return {
                content: [
                    {
                        type: "text",
                        text: `Error communicating with Shopify Dev: ${error instanceof Error ? error.message : String(error)}`,
                    },
                ],
                _meta: {
                    streamId: streamId,
